...
select ok((select count(*) = 4 from pg_policies where schemaname = 'storage' and policyname like 'customer_media_%'), 'four media policies');
select ok((select count(*) = 4 from pg_policies where schemaname = 'storage' and policyname like 'crm_exports_%'), 'four export policies');
