...
| `/automations/recipes`     | Active member                             | Choose a policy-safe recipe        | Prerequisite/policy explanations     |
| `/automations/test-center` | Active member                             | Compare simulation evidence        | Live pilot remains locked            |
| `/automations/[id]`        | Operator/Admin/Owner writes; Viewer reads | Test, activate or emergency pause  | Read-only/not found                  |
| `/analytics`               | Active member                             | Inspect backed operational metrics | Zero values with definitions         |
...
`/automations` and `/analytics`. Browser-supplied workspace IDs never establish
authority.
