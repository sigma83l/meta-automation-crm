...
import { CLINIC, GOLDEN_CASES } from "./fixtures";
...
async function runCase(message: string, eventId: string): Promise<Outcome> {
...
describe.runIf(configured)("golden set: what the assistant may and may not say", () => {
  for (const testCase of GOLDEN_CASES) {
    it(`${testCase.expect === "answers" ? "answers" : "hands off"}: ${testCase.name}`, async (runner) => {
      ...
      const result = await runCase(testCase.message, `golden-${testCase.name}`);
      ...
      // A rate limit means the case did not run, not that it ran and was wrong.
      // Free-tier quota is finite and this set is eight cases of two calls
      // each, so exhausting it is ordinary. Skipping keeps a red result meaning
      // "the assistant said something it should not have", which is the only
      // thing anyone should read this suite for.
      if (failed.some((call) => call.failureKind === "rate_limit")) {
        runner.skip();
        return;
      }
